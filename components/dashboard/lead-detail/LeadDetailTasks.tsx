...
export function LeadDetailTasks({ tasks }: LeadDetailTasksProps) {
  ...
  return (
    ...
    >
      {upcoming.length === 0 ? (
        <p className="text-xs text-zinc-600">Ingen åpne oppgaver.</p>
      ) : (
        <ul className="space-y-3">
          {upcoming.map((task) => (
            <li key={task.id} className={`flex gap-3 p-4 ${dashboardInteractiveCard}`}>
      ...
      {completed.length > 0 && (
        <div className="mt-5 border-t border-zinc-800 pt-4">
          ...
          <ul className="space-y-2">
            {completed.map((task) => (
              <li key={task.id} className="flex items-center gap-2 text-xs text-zinc-600">
